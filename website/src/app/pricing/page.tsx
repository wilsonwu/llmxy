      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
        {data.map((p) => (
          <div key={p.id} className="card">
            ...
            <p className="text-sm text-gray-500">Quota ${(p.quota_cents / 100).toFixed(2)} / {p.duration_days} days</p>
            <a href="/dashboard/topup" className="btn-primary mt-4 w-full">Subscribe</a>
          </div>
        ))}
      </div>
    </div>

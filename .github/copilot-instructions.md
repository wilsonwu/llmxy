...
- Route policies map a public `user_facing_model` to concrete `Model` targets. Always preserve `modality`, `exposed_protocols`, target weights, labels, order, and `scope` semantics. Smart targets without a label belong to the implicit `default` label used for unmatched selection. Round-robin rotates available targets in configuration order using a shared Redis atomic cursor and process-local fail-open cursor. Every strategy may configure one optional fallback model for failed primaries; it may also appear among targets but must not be attempted twice in one request.

...
def upgrade() -> None:
    ...
    op.create_index("ix_oauth_accounts_provider_email", "oauth_accounts", ["provider", "email"])
    ...
    op.create_table(
        ...
        sa.Column("upstream_protocol", sa.String(32), nullable=True),
        sa.Column("prompt_rate", sa.BigInteger, server_default="0"),
        sa.Column("completion_rate", sa.BigInteger, server_default="0"),
        sa.Column("pricing_jsonb", sa.JSON, nullable=False, server_default=sa.text("'{}'")),
        sa.Column("enabled", sa.Boolean, server_default=sa.true()),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now()),
    )
    ...
    op.create_table(
        ...
        sa.Column("exposed_protocols", sa.JSON, nullable=False, server_default=sa.text("'[\"openai\"]'")),
